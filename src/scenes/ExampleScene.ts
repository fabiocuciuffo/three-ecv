...
import { CameraShake } from "../utils/CameraShake"; // Import camera shake utility
...
export class ExampleScene extends Scene implements Lifecycle {
	...
	public constructor({ clock, camera, viewport }: MainSceneParamaters) {
		...
		// Fixer la position de la caméra au début
		...
		wall1Body.quaternion.setFromAxisAngle(
		...
		wall2Body.quaternion.setFromAxisAngle(
			new CANNON.Vec3(0, 1, 0),
			-Math.PI / 2,
		);
	...
	// Ajoutons une méthode pour rétablir la caméra si nécessaire
	private resetCameraPosition(): void {
		const targetPosition = new Vector3(0, 15, 12);
		console.log("Resetting camera to:", targetPosition);
		this.camera.position.copy(targetPosition);
		this.camera.lookAt(0, 0, 0);
		this.cameraShake.resetInitialPosition();
	}
	...
	private handleKeyDown(event: KeyboardEvent): void {
		if (
			...
			// Trigger camera shake when Space is pressed
			if (event.code === "Space") {
				console.log("Space pressed, triggering camera shake");
	...
	private handleKeyUp(event: KeyboardEvent): void {
		if (
			...
			// Stop camera shake when Space is released
			if (event.code === "Space") {
				console.log("Space released, stopping camera shake");
	...
	public update(): void {
		...
		// Update camera shake
		this.cameraShake.update(this.clock.delta);
		...
		if (this.loadedModel && this.modelBody) {
			...
			if (this.keyState.Space) {
				this.particleSystem.emitParticles(this.loadedModel.position.clone());
				 // Removed the camera shake trigger from here since we're handling it in the keydown event
			}
		}

		// Vérifiez que la caméra n'est pas complètement perdue
		...
		if (isNaN(cameraPos.x) || isNaN(cameraPos.y) || isNaN(cameraPos.z) || 
			...
			console.warn("Camera position is invalid:", cameraPos);
